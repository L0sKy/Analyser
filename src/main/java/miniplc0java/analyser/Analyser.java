...
public final class Analyser {
    ...
    private void analyseAssignmentStatement() throws CompileError {
        // 赋值语句 -> 标识符 '=' 表达式 ';'
        var iden = expect(TokenType.Ident);
        // 分析这个语句

        // 标识符是什么？
        String name = iden.getValueString();
        var symbol = symbolTable.get(name);
        if (symbol == null) {
            // 没有这个标识符
            throw new AnalyzeError(ErrorCode.NotDeclared, iden.getStartPos());
        } else if (symbol.isConstant) {
            // 标识符是常量
            throw new AnalyzeError(ErrorCode.AssignToConstant, iden.getStartPos());
        }
        // 设置符号已初始化
        initializeSymbol(name, iden.getStartPos());
        expect(TokenType.Equal);
        analyseExpression();
        expect(TokenType.Semicolon);

        // 把结果保存
        var offset = getOffset(name, iden.getStartPos());
    ...
    private void analyseFactor() throws CompileError {
        ...
        if (check(TokenType.Ident)) {
            ...
            if (symbol == null) {
                ...
                // 标识符没初始化
                throw new AnalyzeError(ErrorCode.NotInitialized, iden.getStartPos());
            }
            var offset = getOffset(name, iden.getStartPos());
